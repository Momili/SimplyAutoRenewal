 ...
 // database connection and table name 
    ...
    function createNew(){        
       $query = "INSERT INTO " . $this->table_name . "
                (UserID,ShopID,ItemID,Title,ImageUrl,Quantity,Views,Likes,LastUpdatedDate,ExpiryDate,ScheduledDateTime,ScheduledDate,ScheduledTime,RenewalStatus,UpdatedTimeStamp,TargetDateTime,LocalDateTime,Unit,Frequency,RenewType,StartDate,EndDate,StartTime,EndTime,NumberOfItems,Sun,Mon,Tue,Wed,Thu,Fri,Sat,TargetTZ,LocalTZ)
                 VALUES
                (:UserID,:ShopID,:ItemID,:Title,:ImageUrl,:Quantity,:Views,:Likes,:LastUpdatedDate,:ExpiryDate,:ScheduledDateTime,:ScheduledDate,:ScheduledTime,:RenewalStatus,:UpdatedTimeStamp,:TargetDateTime,:LocalDateTime,:Unit,:Frequency,:RenewType,:StartDate,:EndDate,:StartTime,:EndTime,:NumberOfItems,:Sun,:Mon,:Tue,:Wed,:Thu,:Fri,:Sat,:TargetTZ,:LocalTZ)";
         try{
            // prepare query            
    ...
    function add_forever_completed($id){
        ...
        $query="INSERT INTO " .$this->table_name ."(UserID,ShopID,ItemID,Title,ImageUrl,Quantity,Views,Likes,LastUpdatedDate,ExpiryDate
                ...
                ,StartDate,EndDate,StartTime,EndTIme,NumberOfItems,Sun,Mon,Tue,Wed,Thu,Fri,Sat,TargetTz,LocalTz)
                SELECT UserID,ShopID,ItemID,Title,ImageUrl,Quantity,Views,Likes,LastUpdatedDate,ExpiryDate
                ,ScheduledDateTime,ScheduledDate,ScheduledTime,'C','". date('Y-m-d H:i:s') ."',TargetDateTime,LocalDateTime,Unit,Frequency,RenewType
                ,StartDate,EndDate,StartTime,EndTIme,NumberOfItems,Sun,Mon,Tue,Wed,Thu,Fri,Sat,TargetTz,LocalTz
                FROM " .$this->table_name ."
                WHERE ID='". $id ."'" ;
    		$stmt = $this->conn->prepare($query);    		
    ...
    function get_scheduled_items(){
        ...
        $query = "SELECT 
                    ...
                    r.StartDate, r.EndDate, r.StartTime, r.EndTime, r.NumberOfItems, r.Sun, r.Mon, r.Tue, r.Wed, r.Thu, r.Fri, r.Sat, r.TargetTZ, r.LocalTZ
                  FROM " . $this->table_name . " r 
                      inner join etsy_users  e on r.ShopID = e.shop_id and r.UserID = e.google_id
                      inner join users u on r.UserID = u.google_id
                  WHERE  (RenewalStatus='S' OR RenewalStatus='R' OR RenewalStatus='F')
    ...
    function delete_all_scheduled($user_id, $shop_id){
        ...
        try{            
            $query ="UPDATE " .$this->table_name ."
                    ...
                    AND ShopID=". $shop_id ."
                    AND (RenewalStatus='S' OR RenewalStatus='R' OR RenewalStatus='F') ";
